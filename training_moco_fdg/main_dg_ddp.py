...
from checkpoint import save_checkpoint, check_local_fdg
...
parser.add_argument("--adaptive", action='store_true', help='use eletrisc gpu adaptive training ')
parser.add_argument("--fdg_finetune", action='store_true', help='use fdg finetune lr decay strategy')
parser.add_argument("--local_rank", default=-1, help='the number of gpus in one machine')
best_acc1 = 0
...
# |-model initilization
    ...
    if args.adaptive:
        model, model_teacher, optimizer, best_acc1, start_epoch = check_local_fdg(args,model,model_teacher,optimizer)
...
# |-Data initilization
    ...
    if args.cross:
        if args.track == '2':
            train_loader, val_loader = get_fourier_train_dataloader_cross('track2','track1', args.root, args.batch_size, use_seg=True,cfg='pairrandomcrop')
        else:
            train_loader, val_loader = get_fourier_train_dataloader_cross('track1','track2', args.root, args.batch_size, use_seg=True,cfg='pairrandomcrop')

    else:
        if args.track == '2':
            train_loader, val_loader_ft, val_loader_test = get_fourier_train_dataloader('track2',args.batch_size, use_seg=True,cfg='pairrandomcrop')
        else:
            train_loader, val_loader_ft, val_loader_test = get_fourier_train_dataloader('track1',args.batch_size, use_seg=True,cfg='pairrandomcrop')
