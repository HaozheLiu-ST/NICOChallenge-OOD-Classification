...
from utils.dataset import get_dataset_test
import json
...
def inference(args):
    net = models.__dict__[args.arch](pretrained=False,num_classes=60)
...
def out_result(result_dict, args):
    if not os.path.isdir(args.save_path+str(args.track)):
        os.makedirs(args.save_path+str(args.track))
...
def voting(args):
    ...
    if args.ckpts == None:
        file_list = os.listdir(args.save_path+args.track)
    else:
        file_list = args.ckpts
    for file in file_list:
        dict_ = json.load(open(args.save_path+args.track+'/'+file, 'r'))
