...
if __name__ == '__main__':
    ...
    pred_dir = create_directory(
        f'./experiments/predictions/{args.experiment_name}@t={args.threshold}@ccam_inference_crf={args.crf_iteration}/')

    # mapping
    import json

    with open('dg_label_id_mapping.json', 'r') as file:
        mapping = json.load(file)

    new_mapping = {v: k for k, v in mapping.items()}
    new_domain_mapping = {v: k for k, v in Domain_dict.items()}
    ...
    ###################################################################################
    dataset = DG_Dataset_For_CCAM(args.data_dir, args.json_file, args.domain)
    ...
    with torch.no_grad():
        length = len(dataset)
        for step, (ori_image, image_id, label, domain) in enumerate(dataset):

            if label is not None:
                add_dir = pred_dir + f'{new_domain_mapping[domain]}/' + f'{new_mapping[label]}/'
                add_ccam_dir = ccam_dir + f'{new_domain_mapping[domain]}/' + f'{new_mapping[label]}/'
            else:
                add_dir = pred_dir
