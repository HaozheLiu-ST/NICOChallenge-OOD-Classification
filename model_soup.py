...
from util3 import accuracy, AverageMeter, ProgressMeter
import torch
import copy
import time
...
parser.add_argument('-m', '--momentum', default=0.5, type=float,
                    metavar='N', help='momentum (default: 0.5)')
...
if args.track == '2':
    _, valid_dataset = get_dataset(512, 'track2', scheme = 'decouple', args.root, args.json_path)
else:
    _, valid_dataset = get_dataset(512, 'track1', scheme = 'decouple', args.root, args.json_path)
...
for idx,model_path in enumerate(args.ckpts):
    if idx == 0:
        ...
        merge_model(model,model_new,lam=args.momentum)
