...
if __name__ == '__main__':
    ...
    log_dir = create_directory('./WSSS/experiments/logs/')
    data_dir = create_directory('./WSSS/experiments/data/')
    model_dir = create_directory('./WSSS/experiments/models/')
    tensorboard_dir = create_directory('./WSSS/experiments/tensorboards/{}/'.format(args.tag))

    log_path = log_dir + '{}.txt'.format(args.tag)
    data_path = data_dir + '{}.json'.format(args.tag)
    model_path = model_dir + '{}.pth'.format(args.tag)
    cam_path = './WSSS/experiments/images/{}'.format(args.tag)
    ...
    train_loader, valid_dataset_for_seg = get_dataset_train('track1', args.data_dir, args.json_file, args.batch_size)
    ...
    # Loss, Optimizer
    ###################################################################################
    # criterion = [SimMaxLossv2(metric='cos', alpha=args.alpha).cuda(), SimMinLoss(metric='cos').cuda(),
    #              SimMaxLossv2(metric='cos', alpha=args.alpha).cuda()]
    ...
    for epoch in range(args.max_epoch):
        for iteration, (images, labels, domains) in enumerate(train_loader):
