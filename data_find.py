...
parser.add_argument("--data_path", default='/data/Track1/', type=str, metavar='PATH',help='path to load data')
...
def dg_with_seg_data_find(data_dir, seg_dir, label_dir, save_dir):
    ...
    for i, file in enumerate(train_path_list):
        ...
        domain = file.split('/')[-3]
        path = file.split(data_dir)[1]
        seg_path = seg_dir+file.split(data_dir+'public_dg_0416/')[1][:-4]+'.png'
...
def ood_with_seg_data_find(data_dir, seg_dir, label_dir, save_dir):
    ...
    for i, file in enumerate(train_path_list):
        label = file.split('/')[-2]
        path = file.split(data_dir)[1]
        seg_path = seg_dir+file.split(data_dir+'public_ood_0412_nodomainlabel/')[1][:-4]+'.png'
        name = '-'.join(file.split(data_dir)[1].split('/'))
        dict ={}
        dict['image_path']=path
        dict['image_seg_path']=seg_path
...
def main():
    ...
    if seg_path == '':
        if args.track == '1':
            dg_data_find(args.data_path,args.label_path,args.json_path)
        else:
            ood_data_find(args.data_path,args.label_path,args.json_path)
    else:
        if args.track == '1':
            dg_data_find(,,)
            dg_with_seg_data_find(args.data_path, args.seg_path, args.label_path, args.json_path)
        else:
            ood_with_seg_data_find(args.data_path, args.seg_path, args.label_path, args.json_path)
