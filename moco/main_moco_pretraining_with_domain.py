...
import moco.loader_with_domain
import moco.loader
import moco.builder_with_domain
...
def main():
    ...
    if args.pretrained:
        args.save_dir = os.path.join(args.save_dir, 'with_domain_pretrained_from_{}'.format(args.pretrained.split('/')[-1]))
    args.save_dir = os.path.join(args.save_dir, 'with_domain_gpus_{}_lr_{}_bs_{}_epochs_{}_path_{}'.format(len(args.gpus.split(',')),
                                                                                     args.lr,
                                                                                     args.batch_size,
                                                                                     args.epochs,
                                                                                     args.data.split('/')[-1]))
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = moco.builder_with_domain.MoCo(
        base_model,
    ...
    train_dataset = moco.loader_with_domain.CustomImageFolder(
        traindir,
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, _, domain) in enumerate(train_loader):
        ...
        # compute output
        output, target = model(im_q=images[0], im_k=images[1], domain=domain)
