...
from fourier_aug import spectrum_decouple_mix_troch, colorful_spectrum_mix_torch
...
def main():
    ...
    args.save_dir = os.path.join(args.save_dir, 'finetune', str(args.input_size), args.arch + f'_{args.track}')
    if args.pretrained:
        args.save_dir = os.path.join(args.save_dir, 'pretrained_from_{}'.format(args.pretrained.split('/')[-1]))
    args.save_dir = os.path.join(args.save_dir, 'fourier_aug_gpus_{}_lr_{}_bs_{}_epochs_{}_path_{}'.format(len(args.gpus.split(',')),
                                                                                     args.lr,
                                                                                     args.batch_size,
                                                                                     args.epochs,
                                                                                     args.data.split('/')[-1]))
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, normalize, args)
...
def train(train_loader, model, criterion, optimizer, epoch, normalize, args):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        if random.random() > 0.5:
            images[0], images[1] = spectrum_decouple_mix_troch(images[0], images[1])
        else:
            images[0], images[1] = normalize(images[0].div(255)), normalize(images[1].div(255))
